...
class SMABacktester():
    ...
    def __init__(self, file, bar_length, SMA_S, SMA_L, start, end, tc):
        self.file = file
        self.SMA_S = SMA_S
        self.SMA_L = SMA_L
        self.start = start
        ...
        self.get_data(self.bar_length)
        self.symbol = self.extract_symbol()
    ...
    def get_data(self, bar = None):
        ''' Retrieves and prepares the data.
        '''
        raw = pd.read_csv("../Historical.Data/{}/{}.csv".format("".join(self.file.split("_")[0]), self.file), parse_dates = ["time"], index_col = "time")

        if bar is not None:
            raw = raw.resample(bar, label="right").last().dropna().iloc[:-1]

        raw = raw.loc[self.start:self.end]
        raw["returns"] = np.log(raw.close / raw.close.shift(1))
        raw.dropna(inplace = True)
        raw["SMA_S"] = raw["close"].rolling(self.SMA_S).mean()
        raw["SMA_L"] = raw["close"].rolling(self.SMA_L).mean()
        raw.dropna(inplace = True)
        self.data = raw
